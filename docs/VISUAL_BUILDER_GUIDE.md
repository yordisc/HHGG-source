...
Method: GET
Authentication: Sesion admin activa (ADMIN_ACCESS_KEY)
...
```php
// Access model used in this project:
- Active admin session (ADMIN_ACCESS_KEY)
...
<fieldset>
    ...
    <div class="form-group">
        ...
        <select id="result_mode" name="result_mode">
            <option value="binary_threshold">Binary (Pass/Fail only)</option>
            <option value="custom">Custom result keys</option>
            <option value="generic">Generic result</option>
        </select>
    </div>
...
| `custom` | Uses custom keys | Branded/custom outcomes |
| `generic` | Generic pass/fail keys | Fallback/default |
...
<div class="live-validation-panel">
    ...
    <button type="submit" :disabled="hasErrors" class="btn btn-primary">
...
<script>
    document
        .getElementById("questions_required")
...
<div class="field-validation">
    <input type="text" name="name" @input="validateField('name')" />
    ...
    <!-- Error message -->
    <p v-if="fields.name.error" class="error">{{ fields.name.errorMessage }}</p>
