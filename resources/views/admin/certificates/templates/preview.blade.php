...
@section('content')
    <section class="mx-auto max-w-6xl rounded-3xl border border-slate-200 bg-white/95 p-6 shadow-sm sm:p-8">
        <div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h1 class="brand-title text-2xl font-bold text-[var(--ink)]">Preview: {{ $template->name }}</h1>
        ...
        <div class="rounded-2xl border border-slate-200 bg-slate-50 p-6">
            <div class="prose max-w-none rounded-xl bg-white p-8 shadow-sm dark:prose-invert">
                {!! str_replace(
                    ['{{nombre}}', '{{fecha}}', '{{serial}}', '{{competencia}}', '{{nota}}'],
                    [$sampleData['nombre'], $sampleData['fecha'], $sampleData['serial'], $sampleData['competencia'], $sampleData['nota']],
