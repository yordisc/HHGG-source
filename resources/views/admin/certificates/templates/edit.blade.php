    ...
    <section class="mx-auto max-w-6xl rounded-3xl border border-slate-200 bg-white/95 p-6 shadow-sm sm:p-8">
        ...
        <form method="POST" action="{{ route('admin.certificates.templates.update', $template) }}" class="space-y-6">
            ...
            <label class="block text-sm font-semibold text-slate-700">
                ...
                <p class="mt-1 text-xs text-slate-500">Puedes pegar un bloque completo con <code>&lt;style&gt;</code>
                    incluido.</p>
                @error('html_template')
                    <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                @enderror
            ...
            <input type="hidden" name="html_template" id="html-template"
                value="{{ old('html_template', $template->html_template) }}">
            <input type="hidden" name="css_template" id="css-template"
                value="{{ old('css_template', $template->css_template) }}">
