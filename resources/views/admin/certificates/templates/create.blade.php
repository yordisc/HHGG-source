    ...
    <section class="mx-auto max-w-6xl rounded-3xl border border-slate-200 bg-white/95 p-6 shadow-sm sm:p-8">
        ...
        <form method="POST" action="{{ route('admin.certificates.templates.store') }}" class="space-y-6">
            ...
            <label class="block text-sm font-semibold text-slate-700">
                ...
                <textarea id="template-content" name="template_content" rows="18"
                    class="mt-1 w-full rounded-xl border border-slate-300 px-4 py-3 font-mono text-xs">{{ old('template_content', $templateContent) }}</textarea>
                <p class="mt-1 text-xs text-slate-500">Pega todo el diseño en un solo bloque. Si incluyes una etiqueta
                    <code>&lt;style&gt;</code>, se separará automáticamente.
                </p>
                @error('html_template')
                    <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                @enderror
            ...
            <input type="hidden" name="html_template" id="html-template" value="{{ old('html_template') }}">
            <input type="hidden" name="css_template" id="css-template" value="{{ old('css_template') }}">
