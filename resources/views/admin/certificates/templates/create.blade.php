        ...
        <form method="POST" action="{{ route('admin.certificates.templates.store') }}" class="space-y-6">
            ...
            <div class="rounded-2xl border border-slate-200 bg-slate-50 p-4 text-sm text-slate-700">
                ...
                <ul class="mt-2 space-y-1 font-mono text-xs">
                    <li>• <code class="text-yellow-600">{{ '{{' }}nombre{{ ' ?>' }}'}}</code> - Nombre del
                        certificado</li>
                    <li>• <code class="text-yellow-600">{{ '{{' }}fecha{{ ' ?>' }}'}}</code> - Fecha de
                        emisión (dd/mm/yyyy)</li>
                    <li>• <code class="text-yellow-600">{{ '{{' }}serial{{ ' ?>' }}'}}</code> - Número de
                        serie único</li>
                    <li>• <code class="text-yellow-600">{{ '{{' }}competencia{{ ' ?>' }}'}}</code> -
                        Nombre de la competencia</li>
                    <li>• <code class="text-yellow-600">{{ '{{' }}nota{{ ' ?>' }}'}}</code> - Resultado
                        (Aprobado, Desaprobado, etc.)</li>
                    <li>• <code class="text-yellow-600">{{ '{{' }}verificacion_url{{ ' ?>' }}'}}</code> -
                        URL firmada de verificación</li>
                    <li>• <code class="text-yellow-600">{{ '{{' }}verificacion_qr{{ ' ?>' }}'}}</code> -
            ...
            <label class="block text-sm font-semibold text-slate-700">
                ...
                <textarea name="html_template" rows="12"
                    class="mt-1 w-full rounded-xl border border-slate-300 px-4 py-3 font-mono text-xs">{{ old('html_template', $defaultHtmlTemplate) }}</textarea>
                @error('html_template')
                    <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                @enderror
            </label>
            ...
            <label class="block text-sm font-semibold text-slate-700">
                ...
                <textarea name="css_template" rows="8"
                    class="mt-1 w-full rounded-xl border border-slate-300 px-4 py-3 font-mono text-xs">{{ old('css_template', '.certificate { font-family: serif; text-align: center; padding: 2rem; }') }}</textarea>
                @error('css_template')
                    <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                @enderror
            </label>
