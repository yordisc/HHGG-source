    ...
    <section class="min-h-screen space-y-6 bg-gradient-to-br from-slate-50 via-white to-slate-50 px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div class="mx-auto max-w-7xl">
            <div class="flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between">
                ...
                <a href="{{ route('admin.questions.create') }}"
                    class="inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-blue-600 to-blue-700 px-6 py-3 font-semibold text-white shadow-lg transition hover:shadow-xl hover:-translate-y-0.5">
                    <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" />
                    </svg>
                    Nueva pregunta
                </a>
                <a href="{{ route('admin.questions.builder') }}"
                    class="inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-purple-600 to-purple-700 px-6 py-3 font-semibold text-white shadow-lg transition hover:shadow-xl hover:-translate-y-0.5">
                    <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 6.732z" />
                    </svg>
                    Constructor visual
                </a>
            </div>
            ...
            @if ($errors->has('csv_file'))
                <div
                    class="mt-6 flex items-start gap-3 rounded-xl border border-rose-200 bg-gradient-to-r from-rose-50 to-rose-50/50 p-4 shadow-sm">
                    <svg class="mt-0.5 h-5 w-5 flex-shrink-0 text-rose-600" fill="currentColor" viewBox="0 0 20 20">
                        <path fill-rule="evenodd"
        ...
        <div class="mx-auto max-w-7xl">
            <form method="POST" action="{{ route('admin.questions.validate.csv') }}" enctype="multipart/form-data"
                ...
                <p class="mb-4 text-sm text-slate-600">Columnas requeridas: <code
                        class="rounded bg-slate-100 px-2 py-1 font-mono text-xs">cert_type, prompt, option_1, option_2,
                        option_3, option_4, correct_option</code></p>
                <p class="mb-4 text-xs text-slate-500">Sugerido: primero usa Plantilla, luego Vista previa, y finalmente
                    confirma la importación.</p>

                <div class="flex flex-col gap-3 sm:flex-row sm:items-end">
                    <div class="flex-1">
                        <label class="block text-sm font-semibold text-slate-700 mb-2">Selecciona archivo CSV</label>
                        <input type="file" name="csv_file" accept=".csv,text/csv"
                            class="w-full rounded-lg border border-slate-300 px-4 py-2 text-sm focus:border-blue-500 focus:ring-2 focus:ring-blue-200"
                            required>
                    </div>
                    <button type="submit"
                        class="inline-flex items-center gap-2 rounded-lg bg-gradient-to-r from-blue-600 to-blue-700 px-6 py-2.5 font-semibold text-white shadow-sm transition hover:shadow-md">
                    ...
                    <a href="{{ route('admin.questions.export.csv', ['cert_type' => $filterType ?: null]) }}"
                        class="inline-flex items-center gap-2 rounded-lg bg-emerald-50 px-6 py-2.5 font-semibold text-emerald-700 shadow-sm border border-emerald-200 transition hover:bg-emerald-100">
                        <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <a href="{{ route('admin.questions.template.csv') }}"
                        class="inline-flex items-center gap-2 rounded-lg bg-amber-50 px-6 py-2.5 font-semibold text-amber-700 shadow-sm border border-amber-200 transition hover:bg-amber-100">
                        <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <div class="mx-auto max-w-7xl">
            <form method="GET" class="space-y-4 rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
                ...
                <p class="text-xs text-slate-500">Combina búsqueda, certificación y estado para ubicar preguntas rápido
                    antes de editar o eliminar.</p>
                    ...
                    <div>
                        ...
                        <select name="cert_type"
                            ...
                            @foreach ($certifications as $slug => $name)
                                <option value="{{ $slug }}" @selected($filterType === $slug)>{{ $slug }}
                                </option>
                            @endforeach
                        </select>
                    </div>
                ...
                <div class="flex items-center gap-3">
                    <button type="submit"
                        ...
                        <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M3 4a1 1 0 000 2h14a1 1 0 100-2H3zM3 12a1 1 0 000 2h14a1 1 0 100-2H3zM3 20a1 1 0 000 2h14a1 1 0 100-2H3z" />
                        </svg>
                        Aplicar filtros
                    </button>
                    <a href="{{ route('admin.questions.index') }}"
                        class="inline-flex items-center gap-2 rounded-lg border border-slate-300 px-6 py-2.5 font-semibold text-slate-700 shadow-sm transition hover:bg-slate-50">
                        <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        ...
                        <tbody class="divide-y divide-slate-100">
                            @forelse ($questions as $question)
                                @php
                                    ...
                                    $questionName =
                                        $questionSlug !== null ? $certifications[$questionSlug] ?? null : null;
                                    $questionPrompt = is_scalar($question->prompt)
                                        ? (string) $question->prompt
                                        : json_encode($question->prompt, JSON_UNESCAPED_UNICODE);
                                    $questionSlugDisplay = is_scalar($questionSlug) ? (string) $questionSlug : 'N/A';
                                @endphp
                                <tr class="transition hover:bg-slate-50">
                                    ...
                                    <td class="px-6 py-4">
                                        <span
                                            ...
                                            {{ $questionSlugDisplay !== '' ? $questionSlugDisplay : 'N/A' }}
                                        </span>
                                    </td>
                                    <td class="px-6 py-4 text-sm text-slate-900">
                                        {{ \Illuminate\Support\Str::limit($questionPrompt, 90) }}</td>
                                    <td class="px-6 py-4">
                                        @if ($question->active)
                                            <span
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex justify-end gap-2">
                                            ...
                                            <form action="{{ route('admin.questions.duplicate', $question) }}"
                                                ...
                                                @csrf
                                                <button type="submit"
                                                    class="inline-flex items-center gap-1 rounded-lg bg-purple-50 px-3 py-1.5 text-xs font-semibold text-purple-700 transition hover:bg-purple-100">
                                                    <svg class="h-4 w-4" fill="none" stroke="currentColor"
                                                        viewBox="0 0 24 24">
                                            ...
                                            <form action="{{ route('admin.questions.destroy', $question) }}"
                                                method="POST" class="inline"
                                                onsubmit="return confirm('¿Confirmas eliminar esta pregunta?');">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit"
                                        ...
                                        <p class="mt-2 text-right text-[11px] text-slate-500">Editar: cambia contenido.
                                            Copiar: crea variante. Eliminar: borra permanentemente.</p>
                                    </td>
                                </tr>
                            @empty
                                <tr>
