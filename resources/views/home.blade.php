    ...
    <div class="home-shell">
        ...
        <section class="mx-auto max-w-7xl">
            <div class="grid gap-12 lg:grid-cols-[1.2fr_0.8fr] lg:items-center">
                ...
                <div>
                    <div class="home-chip">
                        <svg class="ui-icon text-blue-600" fill="currentColor" viewBox="0 0 20 20">
                    ...
                    <div class="mt-8 grid grid-cols-1 gap-4 sm:grid-cols-3">
                        <div
                            class="group rounded-xl border border-slate-200 bg-white/80 p-4 shadow-sm transition hover:shadow-md">
                ...
                <!-- Search Box -->
                <div class="relative">
                    <div class="absolute inset-0 rounded-2xl bg-gradient-to-br from-blue-500/20 to-purple-500/20 blur-2xl">
                    </div>
                    <div class="home-panel p-5 sm:p-7">
                        <div class="mb-6 flex items-center justify-between gap-4">
                            <div>
                                <p class="text-[10px] font-semibold uppercase tracking-wider text-slate-500 sm:text-xs">
                                    {{ __('app.search_label') }}</p>
                                <h2 class="mt-1 text-lg font-bold text-slate-900 sm:text-xl">{{ __('app.search_button') }}
                                </h2>
                            </div>
                            <span
                                class="inline-flex items-center gap-1.5 rounded-full bg-gradient-to-r from-blue-100 to-blue-50 px-2.5 py-1 text-[10px] font-semibold text-blue-700 sm:px-3 sm:text-xs">
                        ...
                        <form method="POST" action="{{ route('search') }}" class="space-y-4">
                            ...
                            <div>
                                <label for="query"
                                    class="block text-sm font-semibold text-slate-700 mb-2">{{ __('app.search_label') }}</label>
                                <input id="query" name="query" type="text" value="{{ old('query') }}"
            ...
            @if (session('search_message'))
                <div
                    class="mt-8 flex items-start gap-4 rounded-xl border border-blue-200 bg-gradient-to-r from-blue-50 to-blue-50/50 p-4 shadow-sm">
                    <svg class="ui-icon mt-0.5 flex-shrink-0 text-blue-600" fill="currentColor" viewBox="0 0 20 20">
                        <path fill-rule="evenodd"
                ...
                @forelse ($certifications as $index => $certification)
                    <article class="home-card group" style="animation: fadeInUp 0.6s ease-out {{ $index * 100 }}ms both;">
                        <div class="relative h-32 overflow-hidden bg-gradient-to-br from-blue-500 to-blue-600 sm:h-40">
                            @if (!empty($certification->featured_image_path))
                                <img src="{{ asset('storage/' . $certification->featured_image_path) }}"
                                    alt="{{ $certification->name }}" class="absolute inset-0 h-full w-full object-cover">
                                <div class="absolute inset-0 bg-slate-900/20"></div>
                            @else
                                <div class="absolute inset-0 bg-gradient-to-br from-blue-400/20 to-transparent"></div>
                            @endif
                            <div
                                class="absolute top-3 right-3 rounded-lg bg-white/20 p-1.5 backdrop-blur-md sm:top-4 sm:right-4 sm:p-2">
                        ...
                        <div class="p-5 sm:p-6">
                            ...
                            <p class="mt-2 text-sm text-slate-600 line-clamp-2">
                                {{ $certification->description ?: __('app.certificate_card_hint') }}</p>
                            <a href="{{ route('quiz.register', ['certType' => $certification->slug]) }}"
                                class="home-primary-btn mt-6 group/btn">
                                {{ __('app.start_exam') }}
                                <svg class="ui-icon transition group-hover/btn:translate-x-1" fill="none"
