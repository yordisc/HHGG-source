...
<body class="relative overflow-x-hidden">
    <div id="app-shell-loader"
        class="fixed inset-0 z-50 flex items-center justify-center bg-[rgba(20,32,51,0.92)] px-6 text-center text-white transition-opacity duration-500">
        <div>
            <div class="mx-auto h-14 w-14 animate-spin rounded-full border-4 border-white/20 border-t-white"></div>
            <p class="mt-5 text-base font-semibold tracking-wide">Despertando los servidores...</p>
            <p class="mt-1 text-sm text-slate-200">Esto puede tardar un momento en el primer acceso.</p>
        </div>
    </div>
    ...
    <header class="sticky top-0 z-20 border-b border-slate-300/80 glass">
        <div
            ...
            <div class="flex flex-wrap items-center gap-2 text-xs font-semibold">
                <a href="{{ route('home') }}"
                    class="rounded-full border border-slate-300 bg-white/90 px-3 py-1.5 text-slate-700 transition hover:border-[var(--accent)] hover:text-[var(--accent)]">{{ __('app.menu_home') }}</a>
                <a href="{{ route('home') }}#search-box"
                    class="rounded-full border border-slate-300 bg-white/90 px-3 py-1.5 text-slate-700 transition hover:border-[var(--accent)] hover:text-[var(--accent)]">{{ __('app.menu_search') }}</a>
                <a href="{{ route('home') }}#quiz-cards"
                    class="rounded-full border border-slate-300 bg-white/90 px-3 py-1.5 text-slate-700 transition hover:border-[var(--accent)] hover:text-[var(--accent)]">{{ __('app.menu_quizzes') }}</a>
            </div>
    ...
    <script>
        (function() {
            ...
            document.addEventListener('livewire:init', hideLoader, {
                once: true
            });
